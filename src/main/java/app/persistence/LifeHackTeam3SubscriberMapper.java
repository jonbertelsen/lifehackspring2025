...
public class LifeHackTeam3SubscriberMapper {
        ...
        try (Connection connection = connectionPool.getConnection();
            ...
            try (ResultSet rs = ps.executeQuery()) {
                if (rs.next()) {
                    ...
                    throw new LifeHackTeam3DatabaseException(null, "Could not find email for the given username.");
                }
            }
        } catch (SQLException ex) {
            throw new LifeHackTeam3DatabaseException(ex, "Could not fetch email from database.");
        }
    }


    // De to metoder der kører koden
    public void executeRemoveSubscriberFromReminder(ConnectionPool connectionPool, String email, String reminderName) throws LifeHackTeam3DatabaseException {
    ...
    public void executeAddSubscriberToReminder(ConnectionPool connectionPool, String email, String reminderName) throws LifeHackTeam3DatabaseException {
        int userId = getUserIdFromEmail(connectionPool, email);
        int reminderId = getReminderIdFromReminderName(connectionPool, reminderName);
    ...
    public boolean verifyUserCredentials(ConnectionPool connectionPool, String username, String password)
        ...
        try (Connection connection = connectionPool.getConnection();
            ...
            throw new LifeHackTeam3DatabaseException(ex, "Could not verify user credentials");
        }
    }
